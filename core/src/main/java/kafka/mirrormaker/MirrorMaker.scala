...
import com.skplanet.kafka.clients.producer.{ProducerConfig, ProducerRecord}
...
object MirrorMaker extends Logging with KafkaMetricsGroup {
  ...
  def main(args: Array[String]) {
    ...
    try {
      ...
      producerProps.setProperty(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, "com.skplanet.kafka.common.serialization.ByteArraySerializer")
      producerProps.setProperty(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, "com.skplanet.kafka.common.serialization.ByteArraySerializer")
  ...
  private class MirrorMakerProducer(val producerProps: Properties) {
    ...
    val producer = new com.skplanet.kafka.clients.producer.KafkaProducer[Array[Byte], Array[Byte]](producerProps)

    def send(record: com.skplanet.kafka.clients.producer.ProducerRecord[Array[Byte], Array[Byte]]) {
      if (sync) {
  ...
  private class MirrorMakerProducerCallback(topic: String, key: Array[Byte], value: Array[Byte])
    extends com.skplanet.kafka.clients.producer.internals.ErrorLoggingCallback(topic, key, value, false) {

    override def onCompletion(metadata: com.skplanet.kafka.clients.producer.RecordMetadata, exception: Exception) {
      if (exception != null) {
